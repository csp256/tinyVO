...
inline ThreadPool::ThreadPool(size_t const threads)
    :   stop(false)
{
    if (threads == 0) {
                ...
                while (true) {
                    std::function<void()> task;
                    {
                        ...
                        this->condition.wait(lock, [this]() -> bool
                            { 
                                return this->stop or not this->tasks.empty(); 
                            });
...
ThreadPool& thread_pool(size_t const n = 0);
